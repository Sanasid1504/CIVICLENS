...
const AuthorityDashboard = () => {
  ...
  const uploadToCloudinary = async (file) => {
    ...
    formData.append("upload_preset", "igjhrgaz");

    const res = await fetch(
      `https://api.cloudinary.com/v1_1/djh53l930/image/upload`,
      {
        method: "POST",
        body: formData
      }
  ...
  useEffect(() => {
    const fetchReports = async () => {
      ...
      try {
        const res = await fetch("https://168.144.68.244.sslip.io/authority/reports", {
  ...
  useEffect(() => {
    const fetchAllHistory = async () => {
      ...
      try {
        ...
        for (let r of reports) {
          const res = await fetch(
            `https://168.144.68.244.sslip.io/authority/report/${r.id}/history`,
            {
              headers: { Authorization: token }
            }
          );
    ...
    if (activeTab === "Audit History" && reports.length) {
      fetchAllHistory();
    }
  }, [activeTab, reports]);
  ...
  try {
    ...
    const res = await fetch(
      `https://168.144.68.244.sslip.io/authority/report/${id}/action`,
      {
        method: "POST",
        headers: {
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-[90] w-64 bg-[#08100d] border-r border-white/5 flex flex-col transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <div className="p-5 border-t border-white/5">
          <button onClick={() => {
            localStorage.clear();
            navigate("/login");
          }} className="w-full flex items-center justify-center gap-2 text-red-500/80 py-2.5 rounded-lg font-bold uppercase text-[10px] border border-red-500/10 hover:bg-red-500/5 transition-all">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 border-b border-white/5 bg-[#08100d]/50 backdrop-blur-md px-4 lg:px-6 flex items-center justify-between gap-3">
          ...
          <div className='flex gap-5'>
            <div className="relative flex-1 gap-5 max-w-md">
            ...
            <select
              ...
              className="text-[10px] bg-[#08100d] border border-white/10 rounded px-2 py-1 ml-2    text-gray-600 w-15 h-9"
            >
          ...
          {activeTab === 'Audit History' ? (
              ...
              <div className="bg-[#08100d] rounded-xl border border-white/5 min-w-[600px] shadow-lg">
                <table className="w-full text-left">
                  <thead className="bg-[#0b1410] text-emerald-500 text-[10px] font-black uppercase tracking-widest border-b border-white/5">
                    <tr><th className="px-5 py-3">Title</th><th className="px-5 py-3">Action</th><th className="px-5 py-3">Remarks</th><th className="px-5 py-3 text-right">Date</th></tr>
                  </thead>
                  <tbody className="divide-y divide-white/5">
                    {auditHistory[0] ? auditHistory.map((log, i) => (
                      <tr key={i} className="hover:bg-white/[0.01]">
                        <td className="px-5 py-3 text-xs font-bold text-white uppercase italic">{log.title}</td>
            ...
            <div className="max-w-7xl mx-auto space-y-4">
              ...
              <div className="grid grid-cols-1 gap-3">
                {reports[0] ? filtered?.map((item, i) => (
                  <div key={item.id} className="bg-white/[0.02] border border-white/5 rounded-xl p-3 flex items-start sm:items-center gap-4 group active:bg-white/[0.05] transition-all cursor-pointer" onClick={() => setSelectedComplaint(item)}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[9px] text-red-400 font-bold">
                        SLA: {getSlaTimeLeft(item.sla_deadline)}
                      </p>
                    </div>
                  </div>
                )) : <>
                  <p className='flex w-full h-full text-gray-500 items-center justify-center'>No reports assigned</p>
                </>}
              </div>
            </div>
          )}
              ...
              <div className="flex-1 lg:w-[400px] p-6 lg:p-10 flex flex-col justify-between bg-[#08100d] overflow-y-auto">
                ...
                <div className="grid grid-cols-1 gap-2 mt-8">
                  {selectedComplaint.status === 'PENDING' && (
                    <ActionButton icon={<FastForward size={18} />} label="Take Action" color="blue" onClick={() => setModalMode('PROGRESS')} />
                  )}
        ...
        {modalMode && (
          <div className="fixed inset-0 z-[110] bg-black/90 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="w-full max-w-sm bg-[#08100d] border border-white/10 rounded-xl p-6 shadow-2xl">
              ...
              {
                modalMode === 'RESOLVE' ? <>
                  <div className='border-2 border-dashed rounded-2xl h-24 mt-5 flex items-center justify-center cursor-pointer bg-[#050d0a]'>
                    <label className="w-full h-full flex items-center justify-center text-gray-500 text-xs cursor-pointer">
              ...
              <div className="flex gap-2 mt-4">
                ...
                <button onClick={() => handleUpdate(modalMode === 'REJECT' ? 'REJECTED' : modalMode === 'RESOLVE' ? 'RESOLVED' : 'IN PROGRESS')} disabled={!statusComment.trim()||uploading} className="flex-1 py-2 rounded-lg text-white text-[10px] font-black uppercase bg-emerald-600 disabled:opacity-20"> {uploading ? "Processing..." : "Confirm"}</button>
              </div>
            </div>
          </div>
        )}
